...
**Version**: `0.1.0-alpha.2` Â· **Updated**: 2026-08-02
...
### âœ… Verified on this machine (real execution)
...
| Cross-platform CI green | GitHub Actions run | pending (CI configured for ubuntu/windows/macos Ã— Node 22/24) |
...
> PATH / a local `openrev-tools` dir) and **skip honestly** when absent â€” they never
...
### âœ… Verified on this machine (Windows)
...
- **UI smoke tests pass** (`npm run test:ui`): launch â†’ import APK via IPC â†’ render
...
- **macOS / Linux builds** â€” require those OSes; CI matrix (ubuntu/macos Ã— Node 22/24)
  is configured but not yet run.
- **Real end-to-end in the webview** â€” the app shell launches, but driving "Open APK â†’
  real file dialog â†’ real IPC â†’ real analysis in the live window" requires interactive
...
- **`zip_reader.ts` coverage ~70%** â€” malformed-archive error branches are under-tested
  (see `COVERAGE_REPORT.md`). A dedicated negative-test file is recommended.
- **No CI performance-regression gate** â€” benchmarks are measured and committed
  (`docs/BENCHMARKS.md`) but not enforced in CI.
- **No coverage gate in CI** â€” coverage is reported, not enforced.
- **No dependabot / dependency update automation** and **no secret-scanning step in CI**
  (e.g. gitleaks) â€” recommended before production.
...
- **No archive size guard for the whole-file hash** â€” a multi-GB APK will be read fully
  into memory for SHA-256 + parse (RSS ~408 MB measured on a 178 MB APK).
- **MCP tool calls run concurrently** â€” a client must await `analyze_target` before
  calling `query_graph_api`/`generate_report`/`search_graph` for dependent data.
- **No auth/rate-limiting** â€” MCP/CLI run with the calling user's privileges on the
...
- `CapabilityEngine` hardcoded `SampleApp.apk` graph + fake counts (`12 Activities`,
  `4 API Endpoints`, `POST /api/v1/auth/login`, `api.example.com`) â†’ real pipeline.
- `remote_execution.ts` hardcoded `cnt_8f93a1` container id â†’ real docker probe/run or
  honest `UNSUPPORTED_FORMAT`.
- `CapabilityRegistry`/`ReportGenerator` fabricated endpoints/counts â†’ real graph data.
- `ElfProvider` canned `x86_64`/`crypto_encrypt` â†’ real ELF header parse.
- `PluginInstaller` fake "downloaded & extracted" â†’ honest not-implemented error.
- Fabricated `docs/BENCHMARKS.md` and `docs/RELEASE_CRITERIA.md` â†’ replaced with
