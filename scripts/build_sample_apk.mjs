/**
 * Build a real, committed sample APK for decompile/decode integration tests.
 *
 * Every byte is extracted from a production APK — no mocks:
 *  - AndroidManifest.xml   : real binary AXML
 *  - resources.arsc        : real resource table
 *  - classes.dex           : real classes14.dex (888 bytes) which contains the
 *                            real com.example.two_rings.MainActivity declared by
 *                            the manifest — jadx decompiles it to real Java in ~1.5s
 *  - res/layout/*.xml      : real binary layout XMLs
 *  - META-INF/MANIFEST.MF  : minimal manifest
 *
 * The sample is small (~76 KB) so jadx/apktool integration tests run in seconds
 * while exercising the REAL success paths.
 */
...
const OUT = join('tests', 'fixtures', 'SampleApp.apk');
const SAMPLE_DEX = 'classes14.dex';

// ---------- read real entries from the source APK ----------
...
function makeEntry(name, data, compress = true) {
  ...
  return {
    ...
    central: { crc, method, storedLen: stored.length, dataLen: data.length, nameBuf }
  };
}

function buildZip(all) {
  const chunks = [];
  const centralChunks = [];
  const offsets = [];
  for (const [name, data, store] of all) {
    const entry = makeEntry(name, data, store !== false);
...
const dex = await readEntry(SAMPLE_DEX);

const layoutNames = [...entries.keys()]
  .filter((n) => n.startsWith('res/layout') && n.endsWith('.xml'))
  .sort();

const all = [
  ['AndroidManifest.xml', await readEntry('AndroidManifest.xml')],
  ['resources.arsc', await readEntry('resources.arsc')],
  ['classes.dex', dex, false], // dex entries are STORED in real APKs; dexlib2 requires it
  ['META-INF/MANIFEST.MF', Buffer.from('Manifest-Version: 1.0\r\n\r\n')]
];
for (const l of layoutNames) {
  all.push([l, await readEntry(l)]);
}

const out = buildZip(all);
await mkdir(dirname(OUT), { recursive: true });
await writeFile(OUT, out);
console.log('SampleApp.apk written:', OUT, out.length, 'bytes,', all.length, 'entries');
console.log('layouts:', layoutNames.length, '| dex size:', dex.length);
