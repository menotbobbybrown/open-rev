...
export class ExtensionHostManager {
  ...
  public async spawnExtensionHost(manifest: ExtensionManifest): Promise<ExtensionProcess> {
    console.log(`[ExtensionHost] Spawning out-of-process worker for plugin: ${manifest.id} (v${manifest.version})`);
  ...
  public terminateExtensionHost(pluginId: string): boolean {
    ...
    console.log(`[ExtensionHost] Terminated process (PID: ${ext.pid}) for plugin ${pluginId}`);
    return true;
  }
